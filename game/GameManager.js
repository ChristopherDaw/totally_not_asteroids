...
var Game = function (canvas, gl) {
    ...
    /*Initialize clouds generated by player*/
    this.clouds = [] 
...
function GameLogic(self, gl, w,h) {
    ...
    self.clouds.forEach(cloud => {
        // Delete enemies older than 1 second
        if (now - cloud.age > 750) {
            self.clouds.shift();
        }
        
        cloudTransform = cloud.translate;

        self.cubeMesh.render(gl, cloudTransform, view, projection, self.playerColor);
    });
...
function AddSmoke(self){
    self.clouds.push(new Cloud(self.playerLocation, Date.now()));
}
